...
if (isset($_POST["email"]) and $_POST["email"] != "") {
 ...
 if (!preg_match("/^(([A-Za-z0-9][A-Za-z0-9-]{0,61}[A-Za-z0-9])|([A-Za-z0-9]+))$/", $domain_array[$i])) {
    ...
    if ( check_email_address($_POST["email"]) == TRUE and $phone !="" and $name !="") {
        ...
        if (isset($_POST["confirm"]) and isset($_POST["hostingdetails"]) and $_POST["hostingdetails"] != "") {
...
ORDER BY hosting_properties.pr_id 
    ...
    while ($row = mysql_fetch_array($result)) {
        ?>   
                    <tr bgcolor="<?php if($colorid%2==0){ echo '#FFFFFF'; }else{ echo '#FFFFFF'; } ?>" >
